<template>
    <div class="container py-5">
      <h2 class="text-orange fw-bold mb-4">{{ $t('besideYou.projects.ready') }}</h2>
      ...
      <div v-else class="text-center text-muted">
        <p>ไม่มีโครงการที่พร้อมอยู่ในขณะนี้</p>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    async created() {
      try {
        ...
        const url = `http://localhost:4000/api/project_recommend/filter?lang=${lang}&category=ready`;
        const response = await axios.get(url);
        this.projects = response.data;
      } catch (error) {
        console.error('Error loading ready projects:', error);
      }
    },
    methods: {
      formatPrice(value) {
